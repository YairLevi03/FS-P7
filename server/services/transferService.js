...
export const executeTransfer = async (userId, transferData) => {
  const { source_account_id, target_account_id, amount, description } = transferData;
  ...
  try {
    ...
    const [targetAccounts] = await connection.query('SELECT * FROM accounts WHERE id = ? FOR UPDATE', [target_account_id]);
    const targetAccount = targetAccounts[0];

    if (!targetAccount) throw { statusCode: 404, message: 'Target account not found.' };
